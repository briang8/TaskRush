...
public class NPC_Enemy : MonoBehaviour {
	...
	// State pattern: behavior for each state is swapped via delegates
	// instead of branching on the enum everywhere else in the class.
	...
	// Linked-list waypoint traversal — each node points to the next.
	...
	void StateUpdate_Inspect(){	
		...
		// Switches to ATTACK once the player is within weapon range and
		// nothing solid is blocking the line between enemy and player.
		if (playerTransform != null) {
			Vector3 toPlayer = playerTransform.position - transform.position;
			float distToPlayer = toPlayer.magnitude;
			if (distToPlayer <= weaponRange) {
				RaycastHit sightHit;
				bool blocked = Physics.Raycast(transform.position, toPlayer.normalized, out sightHit, distToPlayer, hitTestLayer)
					&& !sightHit.collider.CompareTag("Player");
				if (!blocked) {
